...
import { List, City } from '../../Typescript/fiveDaysWeatherType';
...
function* addFiveDaysWeather({
  ...
  function getWeekendWeather(data: any) {
    ...
    const result = data.list.filter(
      (list: any) => list.dt_txt.slice(0, 10) === todayDate,
    );

    // 가져온 데이터의 현재시간을 밀리초로 변경후 현재시간의 밀리초를 뺀 결과의 배열
    const resultMili = result.map((list: any) =>
      Math.abs(Date.parse(list.dt_txt) - todayMilli),
    );

    // 최소값
    const minResultMili = resultMili.sort((a: any, b: any) => a - b)[0];
    const minusMinResultMili =
      resultMili.sort((a: any, b: any) => a - b)[0] * -1;
    ...
    const weekend = data.list.filter((list: any) =>
